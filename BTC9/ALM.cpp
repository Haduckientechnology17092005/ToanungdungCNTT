...
if (normNewDir > 0) {directions.push_back({newDir[0] / normNewDir, newDir[1] / normNewDir});
...
penalty += lagrangeMultipliers[i] * max(gFuncs[i](x1, x2), -
lagrangeMultipliers[i] / (2 * penaltyParameter))
+ penaltyParameter * max(gFuncs[i](x1, x2), -lagrangeMultipliers[i] / (2 *
penaltyParameter));
}
...
cout << "Lagrange multipliers (g constraints): ";
for (double lambda : lagrangeMultipliers) {
cout << lambda << " ";
}
cout << "\nLagrange multipliers (h constraints): ";
for (double h_lambda : lagrangeMultipliersH) {
cout << h_lambda << " ";
}
cout << "\nPenalty parameter: " << penaltyParameter << "\n\n";
...
for (int i = 0; i < gFuncs.size(); ++i) {
lagrangeMultipliers[i] += 2 * penaltyParameter * max(gFuncs[i](minX[0],
minX[1]), -lagrangeMultipliers[i] / (2 * penaltyParameter));
}
for (int j = 0; j < HFuncs.size(); ++j) {
lagrangeMultipliersH[j] += 2 * penaltyParameter * HFuncs[j](minX[0],
minX[1]);
}
...
return 9*pow(x1, 4) + 3*x1*x1*x2 + 2*x1*x2*x2 + 5*pow(x2, 4);
};
vector<function<double(double, double)>> gFuncs = {
[](double x1, double x2) {
return x1*x1 + 2*x1*x2 - 35;
}};
